...
class Cache {
	...
	void swap_index(int i, int j)
	{
		...
		do {
			double[] tmp=head[i].data; head[i].data=head[j].data; head[j].data=tmp;} while(false);
		...
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {
						double tmp=h.data[i]; h.data[i]=h.data[j]; h.data[j]=tmp;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
class Solver {
	...
	static final int BYTE_SPECIES_LENGTH = BYTE_SPECIES.length();
	static final int INT_SPECIES_LENGTH = INT_SPECIES.length();
	...
	VectorMask<Byte> is_free_v(int i) {
		return ByteVector.fromArray(BYTE_SPECIES, alpha_status, i).compare(VectorOperators.EQ, FREE);
	}
...
//		}
		...
		upperBound = BYTE_SPECIES.loopBound(active_size);
		j = 0;
		for(;this.vectorize && this.useMask && j < upperBound; j += BYTE_SPECIES_LENGTH) {
			nr_free = ByteVector.fromArray(BYTE_SPECIES, alpha_status, j).compare(VectorOperators.EQ, FREE).trueCount();
		}
		...
		else
		{
			for(i=0;i<active_size;i++)
				if(is_free(i))
				{
					...
					upperBound = DOUBLE_SPECIES.loopBound(l-active_size);
					j = active_size;
//					for (;this.vectorize && j < upperBound; j += DOUBLE_SPECIES_LENGTH) {
//						DoubleVector.fromArray(DOUBLE_SPECIES, G, j).add(DoubleVector.fromArray(DOUBLE_SPECIES, Q_i, i).mul(alpha_i)).intoArray(G, j);
//					}
					for(;j<l;j++)
						G[j] += alpha_i * Q_i[j];
				}
		   ...
		   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking, boolean vectorize, boolean useMask)
	{
		...
		// initialize active set (for shrinking)
		{
			...
			int upperBound = INT_SPECIES.loopBound(l);
			int i = 0;
			if (this.vectorize && upperBound > 0) {
				int [] index = new int[INT_SPECIES_LENGTH];
				for (int j = 0; j < INT_SPECIES_LENGTH; j++) {
					index[j] = j;
				}

				IntVector index_v = IntVector.fromArray(INT_SPECIES, index, 0);
				for (; i < upperBound; i += INT_SPECIES_LENGTH) {
					index_v.intoArray(active_set, i);
					index_v = index_v.add(INT_SPECIES_LENGTH);
				}
			}
		...
		// initialize gradient
		{
			...
			for (;this.vectorize && i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
				DoubleVector.fromArray(DOUBLE_SPECIES, p, i).intoArray(G, i);
				DoubleVector.zero(DOUBLE_SPECIES).intoArray(G_bar, i);
			}
...
//					}
					...
					if(is_upper_bound(i))
						j = 0;
//						for (;this.vectorize && j < upperBound; j += DOUBLE_SPECIES_LENGTH) {
//							DoubleVector.fromArray(DOUBLE_SPECIES, G_bar, j)
//									.add(DoubleVector.fromArray(
//											DOUBLE_SPECIES, Q_i, j).mul(get_C(i))
//									).intoArray(G_bar, j);
//						}
...
//				svm.info(".");
			...
			int upperBound = DOUBLE_SPECIES.loopBound(active_size);
			int k = 0;
//			for(;this.vectorize && k < upperBound; k += DOUBLE_SPECIES_LENGTH) {
//				DoubleVector Q_i_v = DoubleVector.fromArray(DOUBLE_SPECIES, Q_i, k);
//				DoubleVector.fromArray(DOUBLE_SPECIES, G, k).add(
//						(Q_i_v.mul(delta_alpha_i)).add(Q_i_v.mul(delta_alpha_j))
//				).intoArray(G, k);
//			}
...
class SVR_Q extends Kernel
{
	...
	static private final VectorSpecies<Integer> INT_SPECIES = IntVector.SPECIES_PREFERRED;
	static private final VectorSpecies<Double> DOUBLE_SPECIES = DoubleVector.SPECIES_PREFERRED;
	private final VectorSpecies<Byte> BYTE_SPECIES = ByteVector.SPECIES_PREFERRED;
	static private int INT_SPECIES_LENGTH = INT_SPECIES.length();
	static private int DOUBLE_SPECIES_LENGTH = DOUBLE_SPECIES.length();
	private int BYTE_SPECIES_LENGTH = BYTE_SPECIES.length();
	SVR_Q(svm_problem prob, svm_parameter param, boolean vectorize, boolean useMask)
	{
		super(prob.l, prob.x, param);
		l = prob.l;
		...
		int upperbound = BYTE_SPECIES.loopBound(l);
		int k = 0;
		for (;this.vectorize && k < upperbound; k += BYTE_SPECIES_LENGTH) {
			var zeros = ByteVector.zero(BYTE_SPECIES);
			zeros.add((byte) 1).intoArray(sign, k);
			zeros.sub((byte) 1).intoArray(sign, k+l);
		}
		...
		for(k = 0;k<l;k++)
		{
			index[k] = k;
			index[k+l] = k;
			QD[k] = kernel_function(k,k); // see how to vectorize this loop in an efficient way
			QD[k+l] = QD[k];
		}
...
public class svm {
	...
	static private final VectorSpecies<Double> DOUBLE_SPECIES = DoubleVector.SPECIES_PREFERRED;
	static private final VectorSpecies<Byte> BYTE_SPECIES = ByteVector.SPECIES_PREFERRED;
	static private final int INT_SPECIES_LENGTH = INT_SPECIES.length();
	static private final int DOUBLE_SPECIES_LENGTH = DOUBLE_SPECIES.length();
	static private final int BYTE_SPECIES_LENGTH = BYTE_SPECIES.length();
	...
	public svm(boolean vectorize, boolean useMask) {
		svm.vectorize = vectorize;
		svm.useMask = useMask;
	}
...
//		}
		...
		s.Solve(l, new SVC_Q(prob,param,y), minus_ones, y,
			alpha, Cp, Cn, param.eps, si, param.shrinking, svm.vectorize, svm.useMask);
		...
		for (;svm.vectorize && i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
			sum_alpha_v = sum_alpha_v.add(DoubleVector.fromArray(DOUBLE_SPECIES, alpha, i));
		}
...
//		}
		...
		for (;svm.vectorize &&  i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
			DoubleVector.zero(DOUBLE_SPECIES).intoArray(zeros, i);
		}
		for(;i<l;i++)
			zeros[i] = 0;
		...
		s.Solve(l, new SVC_Q(prob,param,y), zeros, y,
			alpha, 1.0, 1.0, param.eps, si, param.shrinking, svm.vectorize, svm.useMask);
		double r = si.r;
...
//		}
					...
					double[] alpha, Solver.SolutionInfo si)
	{
		...
		Solver s = new Solver();
		s.Solve(2*l, new SVR_Q(prob,param, svm.vectorize, svm.useMask), linear_term, y,
	...
	private static void multiclass_probability(int k, double[][] r, double[] p)
	{
		...
		for (t=0;t<k;t++)
		{
			...
			for (j=0;j<t;j++)
			{
				Q[t][t]+=r[j][t]*r[j][t]; // no viable vectorization as not contiguos memories
				Q[t][j]=Q[j][t];
			}
		...
		for (iter=0;iter<max_iter;iter++)
		{
			...
			pQp=0;
			for (t=0;t<k;t++) // it is possible to vectorize also the outer loop? the difficulty consist in a vectorization inside vectorization
			{
				Qp[t]=0;
			...
			for (t=0;t<k;t++)
			{
				...
				j = 0;
//				for (;svm.vectorize &&  j < upperBound; j += DOUBLE_SPECIES_LENGTH) {
//					(DoubleVector.fromArray(DOUBLE_SPECIES, Qp, j).mul(diff).mul(DoubleVector.fromArray(DOUBLE_SPECIES, Q[t], j))).div(1+diff).intoArray(Qp, j);
//					DoubleVector.fromArray(DOUBLE_SPECIES, p, j).div(1+diff).intoArray(p, j);
//				}
				for (;j<k;j++)
				{
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		// random shuffle
		int upperBound = INT_SPECIES.loopBound(prob.l);
		i = 0;
		int [] index = new int[INT_SPECIES_LENGTH];
		for (; i < INT_SPECIES_LENGTH; i++) {
			index[i] = i;
		}
		i = 0;
		IntVector index_v = IntVector.fromArray(INT_SPECIES, index, 0);
		for (;svm.vectorize &&  i < upperBound; i += INT_SPECIES_LENGTH) {
			index_v.intoArray(perm, i);
			index_v = index_v.add(INT_SPECIES_LENGTH);
		}
		for(;i<prob.l;i++) perm[i]=i;

		for(i=0;i<prob.l;i++)
		{
			int j = i+rand.nextInt(prob.l-i); //TODO figure out how to vectorize with random
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			upperBound = DOUBLE_SPECIES.loopBound(k);
			j = 0;
			for (;svm.vectorize && svm.useMask && j < upperBound; j += DOUBLE_SPECIES_LENGTH) {
				var mask = DoubleVector.fromArray(DOUBLE_SPECIES, subprob.y, j).compare(VectorOperators.GT, 0);
				int count = mask.trueCount();
				p_count += count;
				n_count += (DOUBLE_SPECIES_LENGTH - count);
			}
			for(;j<k;j++)
				if(subprob.y[j]>0)
					p_count++;
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param)
	{
		...
		int upperBound = DOUBLE_SPECIES.loopBound(prob.l);
		i = 0;
		var mae_v = DoubleVector.zero(DOUBLE_SPECIES);
//		for (;svm.vectorize &&  i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
//			var ymv_v = DoubleVector.fromArray(DOUBLE_SPECIES, ymv, i);
//			ymv_v = DoubleVector.fromArray(DOUBLE_SPECIES, prob.y, i).sub(ymv_v);
//			ymv_v.intoArray(ymv, i);
//			mae_v = mae_v.add(ymv_v.abs());
//		}
		
		mae += mae_v.reduceLanes(VectorOperators.ADD);
	...
	private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		...
		if (nr_class == 2 && label[0] == -1 && label[1] == +1)
		{
			...
			int upperBound = INT_SPECIES.loopBound(l);
			i = 0;
			for (;svm.vectorize && svm.useMask && i < upperBound; i += INT_SPECIES_LENGTH) {
				var data_label_v = IntVector.fromArray(INT_SPECIES, data_label, i);
				var mask = data_label_v.compare(VectorOperators.EQ, 0);
				data_label_v = data_label_v.blend(1, mask);
				mask = mask.not();
				data_label_v = data_label_v.blend(0, mask);
				data_label_v.intoArray(data_label, i);
			}
		...
		start[0] = 0;
		for(i=1;i<nr_class;i++) // No vec dependency on i - 1
			start[i] = start[i-1]+count[i-1];
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			...
			int upperBound = DOUBLE_SPECIES.loopBound(prob.l);
			i = 0;
			for (;svm.vectorize && useMask && i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
				nSV += DoubleVector.fromArray(DOUBLE_SPECIES, f.alpha, i).abs().compare(VectorOperators.GT, 0).trueCount();
			}
			for(;i<prob.l;i++)
		...
		else
		{
			...
			i = 0;
			int upperBound = DOUBLE_SPECIES.loopBound(nr_class);
			for (;svm.vectorize &&  i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
				DoubleVector.zero(DOUBLE_SPECIES).add(param.C).intoArray(weighted_C, i);
			}
			for(;i<nr_class;i++)
			...
			for(i=0;i<param.nr_weight;i++)
			{
				int j;
				j = 0;
				upperBound = INT_SPECIES.loopBound(nr_class);
				for (;svm.vectorize && useMask && j < upperBound; j += INT_SPECIES_LENGTH) {
					if (IntVector.fromArray(INT_SPECIES, label, j).compare(VectorOperators.EQ, param.weight_label[i]).anyTrue()) break;
				}
				for(;j<nr_class;j++)
			...
			upperBound = INT_SPECIES.loopBound(nr_class);
			for(;svm.vectorize &&  i < upperBound; i += INT_SPECIES_LENGTH) {
				IntVector.fromArray(INT_SPECIES, model.label, i).intoArray(label, i);
			}
			for(;i<nr_class;i++)
			...
			if(param.probability == 1)
			{
				...
				upperBound = DOUBLE_SPECIES.loopBound(nr_class*(nr_class-1)/2);
				i = 0;
				for (;svm.vectorize &&  i < upperBound; i += DOUBLE_SPECIES_LENGTH) {
					DoubleVector.fromArray(DOUBLE_SPECIES, model.probA, i).intoArray(probA, i);
					DoubleVector.fromArray(DOUBLE_SPECIES, model.probB, i).intoArray(probB, i);
				}
				for(;i<nr_class*(nr_class-1)/2;i++)
				{
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					int k;
					for(k=0;k<ci;k++)
						if(nonzero[si+k]) // cannot vectorize boolean array
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		...
		else
		{
			...
			i = 0;
			int upperBound = INT_SPECIES.loopBound(nr_fold);
			int [] index = new int[INT_SPECIES_LENGTH];
			for (int j = 0; j < INT_SPECIES_LENGTH; j++) {
				index[j] = j;
			}
			var index_v = IntVector.fromArray(INT_SPECIES, index, 0);
//			for (;svm.vectorize &&  i < upperBound; i += INT_SPECIES_LENGTH) {
//				index_v.mul(l).div(nr_fold).intoArray(fold_start, i);
//			}
			for(;i<=nr_fold;i++)
	...
	public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
		   ...
		   model.param.svm_type == svm_parameter.NU_SVR)
		{
			...
			double sum = 0;
			for(i=0;i<model.l;i++)
				// possible to change kernel function to take vector array
		...
		else
		{
			...
			i = 0;
			for (;svm.vectorize &&  i < upperBound; i += INT_SPECIES_LENGTH) {
				IntVector.zero(INT_SPECIES).intoArray(vote, i);
			}
			for(;i<nr_class;i++)
				vote[i] = 0;
